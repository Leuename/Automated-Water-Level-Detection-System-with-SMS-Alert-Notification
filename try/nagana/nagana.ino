...
// Send SMS for sensor error
void sendSMSError(bool sensorWet[]) {
  Serial.println("=== SENDING SENSOR ERROR SMS ===");
  ...
  // Build message showing which sensors are wet/dry
  SIM900A.print("WARNING! Sensors in DISARRAY. Status: ");
  for (int i = 0; i < 5; i++) {
    SIM900A.print(sensorHeights[i]);
    SIM900A.print("ft=");
    SIM900A.print(sensorWet[i] ? "WET" : "DRY");
    if (i < 4) SIM900A.print(", ");
  }
  SIM900A.println(". Check sensors immediately!");
  delay(500);
  ...
  Serial.println("=== SENSOR ERROR SMS SENT ===");
}

// Send SMS when sensors are back to normal
void sendSMSErrorResolved() {
  Serial.println("=== SENDING SENSOR RESOLVED SMS ===");
  ...
  SIM900A.println("INFO: Sensors back to normal operation.");
  delay(500);
  ...
  Serial.println("=== SENSOR RESOLVED SMS SENT ===");
}

// Send SMS for water rising
void sendSMSRise(int sensorIndex) {
  ...
  SIM900A.print("ALERT! Water level RAISED to ");
  SIM900A.print(sensorHeights[sensorIndex]);
  SIM900A.print(" feet.");
  delay(500);
...
void sendSMSDrop(int sensorIndex) {
  ...
  if (sensorIndex == -1) {
    SIM900A.print("ALERT! Water fully DROPPED (below 1 foot).");
  } else {
    SIM900A.print("ALERT! Water level DROPPED to ");
    SIM900A.print(sensorHeights[sensorIndex]);
    SIM900A.print(" feet.");
  }
  delay(500);
